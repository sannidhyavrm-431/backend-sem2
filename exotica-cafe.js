...
function setupUserSession() {
  const session = JSON.parse(localStorage.getItem("exotica_cafe_session"));
...
function handleLogout() {
  localStorage.removeItem("exotica_cafe_session");
...
function setupCartEvents() {
  ...
  const storedCart = localStorage.getItem("exotica_cafe_cart");
  if (storedCart) {
    try {
...
// Global checkout Success Modal close
...
function addToCart(dishId) {
  ...
  localStorage.setItem("exotica_cafe_cart", JSON.stringify(cart));
  renderCartItems();
  updateCartBadge();
...
function renderCartItems() {
  ...
  const cartBreakupSection = document.getElementById("cart-breakup-section");
  ...
  if (cart.length === 0) {
    ...
    if (cartBreakupSection) cartBreakupSection.style.display = "none";
    cartTotalPriceElement.textContent = "₹0";
    checkoutBtn.disabled = true;

    // Reset promo state when cart is cleared
  ...
  if (cartBreakupSection) cartBreakupSection.style.display = "block";
  ...
  if (appliedPromo) {
    ...
    } else if (appliedPromo.code === "RESTO15") {
      discount = Math.round(subtotal * 0.15);
    }
  }

  if (discount > 0) {
    if (discountRow && discountCodeName && cartDiscountElement) {
      discountCodeName.innerHTML = `${appliedPromo.code} <button class="remove-promo-btn" onclick="removePromo()">Remove</button>`;
  ...
  cartTotalPriceElement.textContent = `₹${finalTotal}`;
}
...
function applyPromo() {
  ...
  if (code === "NEW50") {
    appliedPromo = { code: "NEW50", type: "flat", value: 50 };
    promoMessage.textContent = "Coupon NEW50 applied! Flat ₹50 off.";
    promoMessage.className = "promo-message success";
    promoMessage.style.display = "block";
    showToast("Promo NEW50 applied successfully!", "success");
  } else if (code === "RESTO15") {
    appliedPromo = { code: "RESTO15", type: "percent", value: 15 };
    promoMessage.textContent = "Coupon RESTO15 applied! 15% off.";
    promoMessage.className = "promo-message success";
    promoMessage.style.display = "block";
    showToast("Promo RESTO15 applied successfully!", "success");
  } else {
...
function handlePromoKeypress(e) {
  if (e.key === "Enter") {
    applyPromo();
  }
}
...
function handleCheckout() {
  ...
  const orderId = "EXO-" + Math.floor(100000 + Math.random() * 900000);
  ...
  modalOverlay.innerHTML = `
    <div class="modal-content">
      ...
      <div class="modal-desc">
        <p>Thank you for choosing Exotica Cafe, <strong>${session.name}</strong>!</p>
        <p>Your order <strong>${orderId}</strong> has been received and is being prepared with premium care.</p>
        <div class="modal-summary" style="margin: 1.5rem 0; padding: 1rem; background: var(--bg-secondary); border-radius: var(--radius-md); border: 1px solid var(--border-color); text-align: left; font-size: 0.9rem; display: flex; flex-direction: column; gap: 0.4rem; color: var(--text-main);">
          <div style="display: flex; justify-content: space-between;"><span>Items Subtotal:</span><span>₹${subtotal}</span></div>
          ${discount > 0 ? `<div style="display: flex; justify-content: space-between; color: var(--success); font-weight: 600;"><span>Discount Applied (${appliedPromo.code}):</span><span>-₹${discount}</span></div>` : ''}
          <div style="display: flex; justify-content: space-between; font-weight: 700; border-top: 1px dashed var(--border-color); padding-top: 0.4rem; font-size: 1.05rem; margin-top: 0.2rem;"><span>Amount Paid:</span><span>₹${finalTotal}</span></div>
        </div>
        <p>It will arrive hot at your doorstep in approximately 30 minutes.</p>
      </div>
      <button class="modal-btn" onclick="closeCheckoutModal()">Awesome!</button>
    </div>
  `;

  // Clear cart and promo state
  cart = [];
  appliedPromo = null;
  localStorage.removeItem("exotica_cafe_cart");
...
window.handlePromoKeypress = handlePromoKeypress;
